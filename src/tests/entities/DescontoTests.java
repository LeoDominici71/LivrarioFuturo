...
public class DescontoTests {
	...
	public void DescontoTestingA() {
		...
		Livro livroTest = new Livro("X", "X", price);
		DetalhePromocao detP = new DetalhePromocao(new Date("2023/01/10"), new Date("2023/01/15"), livroTest,TipoPromocaoEnum.valueOf("A"), 5);
	...
	public void DescontoTestingB() {
		...
		Livro livroTest = new Livro("X", "X", price);
		DetalhePromocao detP = new DetalhePromocao(new Date("2023/01/10"), new Date("2023/01/20"), livroTest,TipoPromocaoEnum.valueOf("B"), 10);
		List<DetalhePromocao> list = new ArrayList<>();
		detP.desconto(detP.getTipo(),livroTest , detP.getPctPromocao());
		Assertions.assertEquals(expectedValue, detP.desconto(detP.getTipo(), livroTest, detP.getPctPromocao()));
	...
	public void DescontoTestingC() {
		...
		Livro livroTest = new Livro("X", "X", price);
		DetalhePromocao detP = new DetalhePromocao(new Date("2023/01/10"), new Date("2023/01/30"), livroTest,TipoPromocaoEnum.valueOf("C"), 15);
		List<DetalhePromocao> list = new ArrayList<>();
		detP.desconto(detP.getTipo(),livroTest , detP.getPctPromocao());
		Assertions.assertEquals(expectedValue, detP.desconto(detP.getTipo(), livroTest, detP.getPctPromocao()));
